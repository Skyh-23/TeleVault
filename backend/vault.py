...
def import_vault_key(backup_data: bytes, export_password: str) -> bool:
    ...
    # V1 = old unsafe format created by builds before the V2 fix.
    # Keep backward compat so old backups from your 7/31 .exe can still be imported.
    # V1 used the buggy _derive_key_argon2id (hash_secret[:32]) and is rejected as insecure
    # for new backups, but we must still allow IMPORT for migration.
    if magic == _EXPORT_MAGIC:
        ...
        mac = backup_data[-32:]
        # Old buggy derivation - first 32 bytes of PHC string, not raw hash
