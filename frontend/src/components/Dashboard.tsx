...
import { StorageStatsModal } from './dashboard/StorageStatsModal';
import { ShareFileModal } from './dashboard/ShareFileModal';
import { AboutTeleVaultModal } from './AboutTeleVaultModal';
import { VaultRecoveryModal } from './dashboard/VaultRecoveryModal';
...
export function Dashboard({ onLogout }: { onLogout: () => void }) {
    ...
    const handleSyncAll = async () => {
        try {
            ...
            for (const rawFile of res.files) {
                ...
                const normalizedFolderId = folderId === null || folderId === undefined || folderId === '' ? null : Number(folderId);
                const file = {
                    ...rawFile,
                ...
                const current = filesByFolder.get(normalizedFolderId) || [];
                current.push(file);
                filesByFolder.set(normalizedFolderId, current);
            }

            for (const [folderId, files] of filesByFolder) {
                queryClient.setQueryData(['files', folderId], files);
            }

            const targetFolderId = filesByFolder.get(activeFolderId)?.length
                ? activeFolderId
                : [...filesByFolder.entries()].find(([, files]) => files.length > 0)?.[0] ?? null;
            await setActiveFolderId(targetFolderId);
            toast.success(`Synced ${res.files.length} file(s).`);
        } catch (error) {
            console.error('Sync all failed:', error);
            toast.error(`Sync all failed: ${error}`);
        }
    };
    ...
    const { data: allFiles = [], isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await invoke<any[]>('cmd_get_files', { folderId: activeFolderId });
                return res.map(f => ({
                    ...f,
                ...
                // Stale/deleted folder — return empty list instead of crashing
                if (errStr.includes('not found') || errStr.includes('Not Found') || errStr.includes('400')) {
                    return [];
                }
                throw e;
            }
        ...
        enabled: !!store,
        staleTime: 60_000,
        retry: false,  // Don't spam the backend on 400 errors
    });
    ...
    useKeyboardShortcuts({
        ...
        onEnter: handleEnter,
        enabled: !previewFile && !playingFile && !pdfFile && !showMoveModal && !showStorageStats && !shareFile && !showAbout && !showVaultRecovery // Disable when modals are open
    });
    ...
    return (
        ...
        >
            ...
            <AnimatePresence>
                ...
                {showAbout && <AboutTeleVaultModal onClose={() => setShowAbout(false)} />}
                {showVaultRecovery && <VaultRecoveryModal onClose={() => setShowVaultRecovery(false)} />}
                {shareFile && (
                    <ShareFileModal
                        file={shareFile}
                        activeFolderId={activeFolderId}
            ...
            <Sidebar
                ...
                onOpenVaultRecovery={() => setShowVaultRecovery(true)}
            ...
            <main className="flex-1 flex flex-col" onClick={(e) => { if (e.target === e.currentTarget) setSelectedIds([]); }}>
                <TopBar
                    ...
                    onOpenStorageStats={() => setShowStorageStats(true)}
                    viewMode={viewMode}
