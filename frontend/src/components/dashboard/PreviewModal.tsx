...
export function PreviewModal({ file, onClose, onNext, onPrev, currentIndex, totalItems, nextFile, prevFile, activeFolderId }: PreviewModalProps) {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            // Just use the stream URL directly since cmd_get_preview doesn't exist
            const converted = convertFileSrc(file.id.toString(), activeFolderId);
    ...
    useEffect(() => {
        ...
        candidates.forEach((candidate) => {
            ...
            const converted = convertFileSrc(candidate.id.toString(), activeFolderId);
            rememberPreview(key, converted);
            
            // We just pre-calculate the URL; the browser handles the actual prefetching
            const img = new Image();
            img.src = converted;
            img.onload = () => pendingPrefetch.delete(key);
            img.onerror = () => pendingPrefetch.delete(key);
        });
